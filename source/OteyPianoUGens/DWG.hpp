...
class LTIT
{
	...
	float phdel;
	public:
	LTIT():dirty_grdel(true),dirty_phdel(true){//Print("LTIT\n");
	};
	void push(float a){
		cbuf.push(a);
	}
	...
	float phasedelay(float f,float FS){
		...
		if(dirty_phdel){
			...
			phdel = grpdel - omega*::PhaseDelayDerive(omega,KernelB,kernel_sizeB,KernelA,kernel_sizeA);
			dirty_phdel = false;
		}
		return phdel;
	}
...
class LTITv
{
	...
	bool dirty_phdel;
	float phdel;
	LTITv():dirty_grdel(true),dirty_phdel(true){//{//Print("LTITv\n");
	};
	void push(float a){
		cbuf.push(a);
	}
...
class LTITv<1,1>
{
	...
	bool dirty_phdel;
	float phdel;
	LTITv():cbuf(0),cbufout(0),KernelB(0),KernelA(0),dirty_grdel(true),dirty_phdel(true){//Print("LTITv especialized\n");
	};
	void push(float a){
		cbuf = a;
	}
	...
	float phasedelay(float f,float FS){
		...
		if(dirty_phdel){
			float grpdel = groupdelay(f,FS);
			float omega = 2.0*M_PI*f/FS;
			phdel = grpdel - omega*::PhaseDelayDerive(omega,&KernelB,1,&KernelA,1);
			dirty_phdel = false;
		}
		return phdel;
	}
