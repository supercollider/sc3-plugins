...
InterplEnv { 
	...
	*new { arg levels=#[0,1,0], times=#[1,1], curve='lin';
...
InterplPairs : InterplEnv {
	*new {arg pairs, curve;
		var x, y, times, levels;
		curve = curve ? \lin;
		#x, y = pairs.flat.clump(2).flop;
		levels = y;
		times = Array.fill(x.size - 1, {arg i; x[i + 1] - x[i]});
		^InterplEnv.new(levels, times, curve);
		}
	}
