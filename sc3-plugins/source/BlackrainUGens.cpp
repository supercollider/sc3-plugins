...
//    drive	= sc_clip(drive, 0.f, 1.f) * 0.1;
	...
	unit->m_freq = m_freq = (float)(2.f * sin(pi * sc_min(0.25f, cutoff / (SAMPLERATE * 2.f))));
	unit->m_damp = m_damp = sc_min(2.f * (1.f - pow(resonance, 0.25)), sc_min(2.f, 2.f / m_freq - m_freq * 0.5f));
    } else {
	m_freq = unit->m_freq;
	m_damp = unit->m_damp;
    }
